    ...
    $.extend({
        ...
        table: {
            ...
            init: function(options) {
                var defaults = {
                    ...
                    pageSize: 10,
                    pageNumber: 1,
                    pageList: [10, 15, 20, 30, 50, 100, 120, 150, 200, 500],
                    toolbar: "toolbar",
                    loadingFontSize: 13,
                    striped: false,
                    escape: true,
                ...
                $.table.restorePageSize();
            },
            // 保存用户选择的每页记录数到localStorage
            savePageSize: function savePageSize() {
                var optionsIds = $.table.getOptionsIds();
                var pageSize = $(optionsIds).bootstrapTable('getOptions').pageSize;
                localStorage.setItem('bsTablePageSize', pageSize);
            },
            // 从localStorage恢复用户选择的每页记录数
            restorePageSize: function restorePageSize() {
                var optionsIds = $.table.getOptionsIds();
                var pageSize = parseInt(localStorage.getItem('bsTablePageSize'));
                if (!isNaN(pageSize)) {
                    $(optionsIds).bootstrapTable('refreshOptions', {
                        pageSize: pageSize
                    });
                }
            },
            ...
            initEvent: function() {
                ...
                // pageSize变化时保存下来
                $(optionsIds).on('page-change.bs.table', $.table.savePageSize);
            },
            // 当所有数据被加载时触发
        ...
        modal: {
            ...
            msg: function(content, type) {
                if (type != undefined) {
                	top.layer.msg(content, { icon: $.modal.icon(type), time: 3000, shift: 5 });
                } else {
