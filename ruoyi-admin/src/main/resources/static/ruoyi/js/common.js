...
/**
*  处理服务器Ajax响应，用于表单保存信息提示；
**/
function processAjaxReponseJson(response){
	
	let tips = "";
	if(typeof(response["msg"]) != "undefined"){
		if(response["msg"].length > 0){
			tips = response["msg"].replace(new RegExp("\n","gm"),"<br />");
		}
	}

	if(response["code"] === 0) {
		if(tips.length == 0 ) { tips = '配置保存成功！' };
		$.modal.msgSuccess(tips);
		$.modal.closeLoading();
		$.modal.enable();
	}

	if(response["code"] === 500) {
		if(tips.length == 0 ) { tips = '配置保存失败！' };
		console.error('Error saving configuration:', tips);
		$.modal.msgError(tips);
		$.modal.closeLoading();
		$.modal.enable();
	}
